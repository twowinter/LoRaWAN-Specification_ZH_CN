...
Redundancy字段中的** NbTrans**位域，指的是每个上行消息的发送次数(重发次数)，这仅对 "unconfirmed" 消息有作用(因为confirmed类型的消息不需要进行重发)。对于每个数据帧的重发次数默认值为1，有效范围是[1:15]。如果收到**NbTrans**==0，终端需要使用重发次数为1。这个位域可以被NS(网络服务器)用来控制节点上行的 Redundancy 从而获得QOS(服务质量)。在重传帧时终端会像往常一样执行跳频，在每次重传之后终端将会进行等待直到接收窗口过期才进行下次重传，只要在RX1期间收到下行消息，该上行消息则不再进行任何重传。对于 Class A 设备，RX2时隙的接收也是一样处理。
