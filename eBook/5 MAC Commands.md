...
单个数据帧可以包含任何MAC命令序列，要么在FOpts字段中捎带，要么作为独立帧中将**FPort**设成0后放在**FRMPayload**里进行发送。如果采用**FOpts**捎带的方式，MAC命令不进行加密并且长度绝对不得超过15字节。如果采用独立帧放在**FRMPayload**进行发送 的方式，那就必须采用加密方式，并且绝对不得超过**FRMPayload**的最大长度。

> 注意：如果MAC命令不想被窃听，那就必须以独立帧形式放在FRMPayload中进行发送。

每个MAC命令是由1字节命令码(CID)跟着一段可能为空的字节特定命令序列组成的。
...
<table>
   ...
   <tr>
      ...
      <td></td>
      <td>x</td>
	  <td>LinkCheckReq的回复。包含接收信号强度，指明终端接收质量</td>
   </tr>
   <tr>
      <td>0x03</td>
      <td>LinkADRReq</td>
...
> 注意：MAC命令的长度虽然没有明确给出，但是MAC执行层必须要知道。因此未知的MAC命令无法被忽略，且前面未知的MAC命令会终止MAC命令的处理队列。所以建议按照LoRaWAN协议介绍的MAC命令来处理MAC命令。这样所有基于LoRaWAN协议的MAC命令都可以被处理，即使是更高版本LoraWAN协议中的MAC命令。

---

> 注意：由网络服务器调整的任何值（例如，RX2、新的或已调整的信道定义）仅在终端设备的下一次连接之前有效。因此，在每个成功的连接过程之后，终端设备再次使用默认参数，并由网络服务器根据需要重新调整值。
...
解调裕度(**Margin**)是一个范围为0~254的8位无符号整数，表示成功接收最新的**LinkCheckReq**命令的链路裕度(单位为dB)。若Margin值为"0"则意味着数据帧是在解调层进行接收(0 dB或者没有裕度)但是例如Margin值为"20"时则意味着数据帧到达在解调层之上20dB的网关。
...
所请求的数据速率(**DataRate**)和发射功率(**TXPower**)是根据区域规定，体现在[LoRaWAN协议中文版_配套文件 地区参数(物理层) ](http://blog.csdn.net/iotisan/article/details/55056092)中。命令中的发射功率字段指的是设备可操作的最大发射功率。如果命令中的发射功率高于终端实际发射功率的最大值，终端也要应答成功，这种情况下，将终端的发射功率尽可能提高到最大值。信道掩码(ChMask)字段指示了上行链路的可用信道，其中bit0为**最低有效位**（Least Significant bit)。 
...
Redundancy字段中的** NbTrans**位域，指的是每个上行消息的发送次数(重发次数)，这仅对 "unconfirmed" 消息有作用(只用于设置"unconfirmed"类型的数据包重发次数)。对于每个数据帧的重发次数默认值为1，有效范围是[1:15]。如果收到**NbTrans**==0，终端需要使用重发次数为1。这个位域可以被NS(网络服务器)用来控制节点上行的 Redundancy 从而获得QOS(服务质量)。在重传帧时终端会像往常一样执行跳频，在每次重传之后终端将会进行等待直到接收窗口过期才进行下次重传，只要在RX1期间收到下行消息，该上行消息则不再进行任何重传。对于 Class A 设备，RX2时隙的接收也是一样处理。

ChMaskCntl 位域和之前定义的 ChMask 字段有关，它控制了ChMask所指定的16个信道块。也可以通过特定调制对信道进行全局的打开或关闭。这个位域的使用是根据区域规定，体现在[LoRaWAN协议中文版_配套文件 地区参数(物理层) ](http://blog.csdn.net/iotisan/article/details/55056092)中。

NS(网络服务器)可能会在单个下行帧中包含多个 LinkAdrReq 命令。终端为了配置Channel mask，将会按照下行消息中的命令块的顺序，逐一地处理所有的 LinkAdrReq 消息。 终端可能会接收或者拒绝命令块中所有 channel mask 的控制，在每个LinkAdrAns 命令块中提供连续的 Channel Mask ACK 状态，来指示相应的 channel mask 接受与否。 终端在连续命令块时只处理最后一个消息中的 DataRate, TXPower 和 NbTrans 字段，因此这些参数将会决定终端的全局状态。终端需要在每一个 LinkAdrAns 命令中体现 ACK 状态，来指示对这些最终设置的接受与否。
...
DutyCycleReq命令是用于网络协调器去限制终端的**最大聚合发射占空比**。**最大聚合发送占空比**与所有子带上的发射占空比有关。
...
**RXParamSetupReq**命令可以对每个上行消息之后的第二接收窗口的频率以及速度率进行改变。该命令还可以对上行消息数据率和RX1接收窗口时隙的下行消息数据率之前的偏移量进行改变。
...
**RX1DRoffset**位域设置了在与终端的上行消息数据率与第一次接收时隙（RX1）通信时的下行消息数据率的偏移量。默认情况下偏移量为0（意思就是上行数据率与下行数据率相等)。偏移量用于考虑一些地区的基站的最大功率密度限制和平衡上下链路和下行链路的无线电路裕度。

**RX2DataRate**位域定义了第二接收窗口的下行链路数据率，遵循与**LinkADRReq**命令相同的规则（例如，0表示DR0/125kHz)。**Frequency**位域所设置的是第二接收窗口所使用信道的频率，该频率按照与**NewChannelReq**命令相同的规则进行定义。

终端使用**RXParamSetupAns**命令对**RXParamSetupReq**命令进行应答。**RXParamSetupAns**命令应该添加在所有的上行链路数据帧的**Fopt**字段中直到终端接收到一个Class A类型的下行链路数据帧。
这样就可以保证即使在上行链路帧丢失的情况之下，网络服务器总是可以知道终端所使用的下行链路参数。
...
<table>
   ...
   <tr>
      ...
      <td>上行数据率与RX1时隙的下行数据率的偏移量不在允许的范围之内</td>
...
通过**DevStatusReq**命令，NS(网络服务器)可以获取终端的状态信息。该命令无载荷。一旦终端收到**DevStatusReq**命令，则会以**DevStatusAn**命令进行回复。
...
**Margin**是最近一次成功接收**DevStatusReq**命令的解调信噪比(该值必须是四舍五入到最近的整数值，单位为dB）。解调信噪比是6位的有符号整数（最小值为 -32，最大值为31）。
...
**NewChannelReq**命令可以用于修改现有的双向信道或者创建一个新的信道。这个命令设置了新的信道的中心频率还有上行数据率的可用范围：
...
**Freq**位域是一个24位无符号整数。以Hz为单位的实际信道频率为（100×**Freq**），其中表示低于100MHz的频率的值保留供将来使用。**Freq**可以设置一个信道的频率从100MHz~1.67GHz之间，但必须以100Hz为单位(因为实际信道频率为（100×**Freq**））。终端必须检测该频率是否能被无线硬件所使用，若不行则需返回错误。

**DrRange**位域规定了这个信道所允许的上行数据率范围。该位域 被分为两个4位字段：
...
按照[章节5.2](#5.2)所定义的规则,最小数据率**MinDR**字段规定了这个信道所允许的最低上行数据率。例如0表示DR0/125kHz。类似的，最大数据率**MaxDR**规定了最高上行数据率。例如，若**DrRange**=0x77则表示一个信道只允许50kbps的GFSK；若**DrRange**=0x50表示一个信道支持DR0/125kHz到DR5/125kHZ
的频率范围。
...
**DlChannelReq**命令允许服务器在RX1时隙使用不同的下行链路频率。这个命令可以适用于所有支持**NewChannelReq**命令的地理区域(例如欧盟和中国，但是不适用于美国和澳大利亚，具体详见[LoRaWAN协议中文版_配套文件 地区参数(物理层) ](http://blog.csdn.net/iotisan/article/details/55056092)。
...
**Chlndex**是下行频率被修改的信道的索引。

**Freq**位域是24位的无符号整数。。以Hz为单位的实际信道频率为（100×**Freq**），其中表示低于100MHz的频率的值保留供将来使用。终端必须检测该频率是否能被无线硬件所使用，若不行则需返回错误。
...
该命令只需要在特定的可调节地区进行使用。具体请参考[LoRaWAN协议中文版_配套文件 地区参数(物理层) ](http://blog.csdn.net/iotisan/article/details/55056092)。
